...
use OpenDxp\Extension\Document\Areabrick\AreabrickInterface;
use OpenDxp\Extension\Document\Areabrick\AreabrickManager;
 ...
 * @internal
 */
final readonly class AreabrickPass implements CompilerPassInterface
{
    ...
    {
        $config = $container->getParameter('opendxp.config');

        $areabrickManager = $container->getDefinition(AreabrickManager::class);
        $areabrickLocator = $container->getDefinition('opendxp.document.areabrick.brick_locator');

        $taggedServices = $container->findTaggedServiceIds('opendxp.area.brick');
        ...
        foreach ($taggedServices as $id => $tags) {
            ...
            foreach ($tags as $tag) {
                ...
                // e.g. { name: opendxp.area.brick, id: blockquote }
