...
use OpenDxp\Config;
use OpenDxp\Event\Cache\FullPage\CacheResponseEvent;
use OpenDxp\Event\Cache\FullPage\PrepareResponseEvent;
use OpenDxp\Event\Cache\FullPage\PrepareTagsEvent;
...
class FullPageCacheListener
{
    ...
    public function onKernelRequest(RequestEvent $event): void
    {
        ...
        if (isset($_REQUEST['opendxp_cache_tag_suffix'])) {
            $tags = $_REQUEST['opendxp_cache_tag_suffix'];
            if (is_array($tags)) {
        ...
        $this->defaultCacheKey = 'output_' . md5(\OpenDxp\Tool::getHostname() . $requestUri . $appendKey);
        $cacheKeys = [
        ...
        if ($cacheItem) {
            ...
            $response = $cacheItem;
            $response->headers->set('X-OpenDxp-Output-Cache-Tag', $cacheKey, true);
            $cacheItemDate = strtotime($response->headers->get('X-OpenDxp-Cache-Date'));
    ...
    public function onKernelResponse(ResponseEvent $event): void
    {
        ...
        if ($this->enabled && $response->getStatusCode() === 200 && $this->defaultCacheKey) {
            try {
                ...
                $response->headers->set('X-OpenDxp-Cache-Date', $now->format(DateTimeInterface::ATOM));
