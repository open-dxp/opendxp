...
namespace OpenDxp\Bundle\InstallBundle\Command;
...
use OpenDxp\Console\ConsoleOutputDecorator;
...
class InstallCommand extends Command
{
    ...
    private function getOptions(): array
    {
        ...
        foreach (array_keys($options) as $name) {
            $options[$name]['env'] = 'OPENDXP_INSTALL_' . strtoupper(str_replace('-', '_', $name));
        }
    ...
    protected function initialize(InputInterface $input, OutputInterface $output): void
    {
        ...
        $this->io = new OpenDxpStyle($input, $output);
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        if ($input->isInteractive() && !$this->io->confirm('This will install OpenDxp with the given settings. Do you want to continue?')) {
            return 0;
        }
        ...
        $this->installer->setCommandLineOutput(new OpenDxpStyle(
            $input,
        ...
        $this->io->success('OpenDxp was successfully installed');
