...
use OpenDxp\Bundle\InstallBundle\BundleConfig\BundleWriter;
use OpenDxp\Bundle\InstallBundle\Event\BundleSetupEvent;
use OpenDxp\Bundle\InstallBundle\Event\InstallerStepEvent;
use OpenDxp\Bundle\InstallBundle\Event\InstallEvents;
use OpenDxp\Bundle\InstallBundle\SystemConfig\ConfigWriter;
...
class Installer
{
    ...
    public const array INSTALLABLE_BUNDLES = [
        ...
        'OpenDxpSeoBundle' => OpenDxpSeoBundle::class,
        'OpenDxpSimpleBackendSearchBundle' => OpenDxpSimpleBackendSearchBundle::class,
        'OpenDxpStaticRoutesBundle' => OpenDxpStaticRoutesBundle::class,
        'OpenDxpTinymceBundle' => OpenDxpTinymceBundle::class,
        'OpenDxpUuidBundle' => OpenDxpUuidBundle::class,
        'OpenDxpWordExportBundle' => OpenDxpWordExportBundle::class,
        'OpenDxpXliffBundle' => OpenDxpXliffBundle::class,
    ...
    public function setCommandLineOutput(OpenDxpStyle $commandLineOutput): void
    {
    ...
    private function runInstall(array $dbConfig, array $userCredentials, Connection $db): array
    {
        ...
        if (isset($_ENV['OPENDXP_KERNEL_CLASS'])) {
            $kernel = $_ENV['OPENDXP_KERNEL_CLASS'];
        }
    ...
    private function runCommand(array $arguments, string $taskName): void
    {
        ...
        try {
            array_splice($arguments, 0, 0, [
                Console::getPhpCli(),
                OPENDXP_PROJECT_ROOT . '/bin/console',
            ]);
            ...
            $process->setWorkingDirectory(OPENDXP_PROJECT_ROOT);
            $process->run();
    ...
    private function markMigrationsAsDone(): void
    {
        ...
        $this->runCommand([
            ...
            '--all', '--add', '--prefix=OpenDxp\\Bundle\\CoreBundle', '-n', '-q',
    ...
    {
        $this->runCommand([
            'opendxp:deployment:classes-rebuild',
            '-c',
    ...
    public function setupDatabase(Connection $db, array $userCredentials, array $errors = []): array
    {
        ...
        if (OpenDxp::getKernel() instanceof \OpenDxp\Kernel) {
    ...
    {
        return glob(OPENDXP_PROJECT_ROOT . '/dump/*.sql*');
    }
    ...
    protected function createOrUpdateUser(Connection $db, array $config = []): void
    {
        ...
        $db->insert('users', [
            ...
            'password' =>\OpenDxp\Tool\Authentication::getPasswordHash($settings['username'], $settings['password']),
            'active' => 1,
            'admin' => 1,
            'type' => 'user',
            'language' => 'en',
