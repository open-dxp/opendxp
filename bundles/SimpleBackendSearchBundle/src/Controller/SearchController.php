...
namespace OpenDxp\Bundle\SimpleBackendSearchBundle\Controller;
...
use OpenDxp\Bundle\SimpleBackendSearchBundle\Event\AdminSearchEvents;
        ...
        foreach ($hits as $hit) {
            ...
            if ($element->isAllowed('list')) {
                $data = [
                    ...
                    'fullpathList' => htmlspecialchars($this->shortenPath($element->getRealFullPath())),
                    'iconCls' => 'opendxp_icon_asset_default',
                ];

                $this->addAdminStyle($element, ElementAdminStyleEvent::CONTEXT_SEARCH, $data);

                $validLanguages = \OpenDxp\Tool::getValidLanguages();

                $data['preview'] = $this->renderView(
                    '@OpenDxpAdmin/searchadmin/search/quicksearch/' . $hit->getId()->getType() . '.html.twig', [
                        'element' => $element,
