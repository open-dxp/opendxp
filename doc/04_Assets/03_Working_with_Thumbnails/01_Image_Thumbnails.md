...
For images, OpenDXP offers an advanced thumbnail-service also called 'image-pipeline'. It allows you to transform images
in unlimited steps to the expected result. You can configure them in _Settings_ > _Thumbnails_.

With this service every image which is stored as an asset can be transformed. OpenDXP doesn't support to modify images
which are not stored as an asset inside OpenDXP.
...
> Using ImageMagick OpenDXP can support hundreds of formats including: AI, EPS, TIFF, PNG, JPG, GIF, PSD, etc.
> Not all formats are allowed out of the box. To extend the list [see](./README.md#allowed-formats).

To use the thumbnailing service of OpenDXP, you have to create a transformation pipeline first. To do so, open
...
an `\OpenDxp\Model\Asset\Image\Thumbnail` object. The thumbnail object's `__toString()` method returns the path to the thumbnail file, for example:
...
// preferred alternative - let OpenDXP create the whole image tag
...
OpenDXP offers the method `getHTML(array $options)` to get a ready to use `<picture>` tag for your thumbnail. When a `<picture>` element is not needed or desired an `<img />` element can used by calling `getImageTag(array $options)` instead.

You can configure the generated markup with the following options:

| Name                           | Type     | Description                                                                                                                        |
|--------------------------------| -------- |------------------------------------------------------------------------------------------------------------------------------------|
| `disableWidthHeightAttributes` | bool     | Width & height attributes are set automatically by OpenDXP, to avoid this set this option (eg. to true => isset check)             |
...
It is also possible to define an alternative metadata to be used as `alt`, `copyright`, `title` values (eg. by defining `opendxp.assets.metadata.alt` in the configuration) that would have used when the inline options are not passed.
...
{{ opendxp_asset_by_path('/path/to/image.jpg').thumbnail('myThumbnail').html|raw }}

/* ... with some additional options */
{{ opendxp_asset_by_path('/path/to/image.jpg').thumbnail('myThumbnail').html({
    pictureAttributes: {
        data-test: "my value"
    },
    disableImgTag: true
...
<div>
    <p>
        {{ opendxp_image('myImage', {'thumbnail': 'myThumbnail'}) }}
    </p>
</div>

/* Use without pre-configured thumbnail */
{{ opendxp_image('myImage', {
    'thumbnail': {
        'width': 500,
        'aspectratio': true,
        'interlace': true,
...
{{ opendxp_asset_by_path('/path/to/image.jpg').thumbnail({'width': 500, 'format': 'png'}).html|raw}}
```

## Advanced Examples

OpenDXP returns an `\OpenDxp\Model\Asset\Image\Thumbnail` object when calling `$asset->getThumbnail("thumb-name")`
...
OpenDXP supports ICC color profiles to get better results when converting CMYK images (without embedded color profile)
to RGB.

Due licensing issues OpenDXP doesn't include the color profiles (\*.icc files) in the download package, but
...
(eg. `/usr/share/color/icc`). Then configure the path in the OpenDXP config file:
...
OpenDXP auto-generates a thumbnail if requested but doesn't exist on the file system and is directly called via it's file path (not using any of
...
(`/examples/panama/` is the path to the source asset, `6644` is the ID of the source asset, `contentimages` is the name of the thumbnail configuration, `img_0037.jpeg` the filename of the source asset) directly in your browser. Now OpenDXP checks
...
For performance reasons, OpenDXP doesn't generate the thumbnail image directly when calling `getThumbnail()`
...
The following is only necessary in special use-cases like Web-to-Print, in typical web-based cases, OpenDXP
...
```twig
{{ opendxp_image('myImage', {'thumbnail': 'contentimages'}) }}
```
...
```twig
{{ opendxp_image('myImage', {thumbnail: {
    width: 250,
    contain: true,
    highResolution: 2
}}) }}
...
OpenDXP also utilizes this functionality internally to provide the automatically added high.res support on `<img>` and `<picture>` tags.
...
OpenDXP will then dynamically generate the thumbnails accordingly.

## Media Queries in Thumbnail Configuration

If your're using media queries in your thumbnail configuration OpenDXP automatically generates a `<picture>` tag when calling `$asset->getThumbnail("example")->getHtml()`.
...
OpenDXP supports focal points on images, which are considered when images are automatically cropped.
        ...
        image:
            thumbnails:
                auto_formats:
                    ...
                    # the following config is used as the default by OpenDXP
...
It is possible to manually specify the used image processing adapter by OpenDXP. 
You can choose from `Imagick` or `GD`, the default is auto-detected, based on the availability of `imagick` PECL extension. 
It is also possible to implement your own adapter, by implementing `OpenDxp\Image\AdapterInterface`.

To specify the used image adapter, please use the following service configuration: 

```yaml
services: 
    OpenDxp\Image\AdapterInterface:
        alias: OpenDxp\Image\Adapter\GD
        public: true
```
