...
OpenDXP provides an object orientated PHP API to work with Objects. There are several generic functionalities
provided by OpenDXP and for each OpenDXP object class OpenDXP generates corresponding PHP classes for working
...
$newObject->setKey(\OpenDxp\Model\Element\Service::getValidKey('New Name', 'object'));
...
not only programmatically but also through the OpenDXP object csv export.

Object listings are a simple way to retrieve objects from OpenDXP while being able to filter and sort data along that
...
An object listing class is created automatically for each class defined in OpenDXP. Objects for the class `Myobject`
...
$entries = new \OpenDxp\Model\DataObject\Myclassname\Listing();
...
$list = \OpenDxp\Model\DataObject\News::getList(["unpublished" => true]);
...
\OpenDxp\Model\DataObject::setHideUnpublished(true);

// force to return all objects including unpublished ones, even if setUnpublished is set to false
\OpenDxp\Model\DataObject::setHideUnpublished(false);
```
...
$list = \OpenDxp\Model\DataObject\Collectiontest::getList([
   "fieldCollections" => [
...
<nav aria-label="Pagination">
    <ul class="pagination justify-content-center">
        {%  if(paginationVariables.previous is defined) %}
            <li class="page-item">
                <a class="page-link prev" href="{{  opendxp_url({'page': paginationVariables.previous}) }}" aria-label="Previous">
        ...
        {%  for page in paginationVariables.pagesInRange %}
            ...
            {%  else %}
                <li class="page-item"><a class="page-link" href="{{  opendxp_url({'page': page}) }}">{{ page }}</a></li>
            {%  endif %}
        ...
        {%  if(paginationVariables.next is defined) %}
            <li class="page-item">
                <a class="page-link next" href="{{  opendxp_url({'page': paginationVariables.next}) }}" aria-label="Next">
...
use OpenDxp\Model\DataObject\Car\Listing;
