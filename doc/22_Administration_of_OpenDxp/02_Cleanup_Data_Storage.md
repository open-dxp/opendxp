...
In general OpenDXP is quite maintenance-free in terms of cleaning up the filesystem from temporary files, log files, 
...
OpenDXP stores the meta-data for version in the database, however, the data itself is stored on the filesystem 
(`var/versions`) as compressed files (gzip) to keep the database as lean as possible. 
It's important to know that OpenDXP stores complete dumps of the data at the time the version is created, it doesn't 
...
```bash
./bin/console opendxp:maintenance -j versioncleanup
```
...
## Logging Data
All logging information is located in `var/log/`. OpenDXP rotates & compresses and cleans up the logs automatically: 
...
maintenance command: `./bin/console opendxp:maintenance -J logmaintenance`

## Temporary Files
OpenDXP stores temporary files in 2 different locations, depending on whether they are public accessible or not.   
...
## Recycle Bin
Deleting items in OpenDXP moves them to the recycle bin first. The recycle bin works quite similar to the versioning, 
...
```bash
./bin/console  opendxp:recyclebin:cleanup --older-than-days=60
```
