 ...
 * @internal
 *
 * Locates configs from bundles if Resources/config/opendxp exists.
 *
 ...
 *
 *  - Resources/config/opendxp/config_dev.php
 *  - Resources/config/opendxp/config_dev.yaml
 *  - Resources/config/opendxp/config_dev.xml
 *
 * If the previous lookup didn't return any results, it will fall back to:
 *
 *  - Resources/config/opendxp/config.php
 *  - Resources/config/opendxp/config.yaml
 *  - Resources/config/opendxp/config.xml
 */
class BundleConfigLocator
{
    ...
    public function locate(string $name): array
    {
        ...
        foreach ($this->kernel->getBundles() as $bundle) {
            ...
            if (!is_dir($dir = $bundlePath.'/Resources/config/opendxp') && !is_dir($dir = $bundlePath.'/config/opendxp')) {
                continue;
            }
