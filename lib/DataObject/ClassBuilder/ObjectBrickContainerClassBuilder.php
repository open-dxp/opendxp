...
class ObjectBrickContainerClassBuilder implements ObjectBrickContainerClassBuilderInterface
{
    public function buildContainerClass(Definition $definition, ClassDefinition $classDefinition, string $fieldName, array $brickKeys): string
    {
        ...
        $cd .= "\n\n";
        $cd .= 'class ' . $className . ' extends \\OpenDxp\\Model\\DataObject\\Objectbrick {';
        $cd .= "\n\n";
        ...
        foreach ($brickKeys as $brickKey) {
            $cd .= 'protected \\OpenDxp\\Model\\DataObject\\Objectbrick\\Data\\' . ucfirst($brickKey) . '|null $' . $brickKey . " = null;\n\n";

            $cd .= '/**' . "\n";
            $cd .= '* @return \\OpenDxp\\Model\\DataObject\\Objectbrick\\Data\\' . ucfirst($brickKey) . "|null\n";
            $cd .= '*/' . "\n";
            $cd .= 'public function get' . ucfirst($brickKey) . '(bool $includeDeletedBricks = false)' . "\n";
            $cd .= '{' . "\n";

            if ($classDefinition->getAllowInherit()) {
                $cd .= "\t" . 'if(!$this->' . $brickKey . ' && \\OpenDxp\\Model\\DataObject::doGetInheritedValues($this->getObject())) { ' . "\n";
                $cd .= "\t\t" . 'try {' . "\n";
                ...
                $cd .= "\t\t\t\t\t" . '$brickType = "\\\OpenDxp\\\Model\\\DataObject\\\Objectbrick\\\Data\\\" . ucfirst($parentBrick->getType());' . "\n";
            ...
            $typeDeclaration = '\\OpenDxp\\Model\\DataObject\\Objectbrick\\Data\\' . ucfirst($brickKey);
            $cd .= '/**' . "\n";
