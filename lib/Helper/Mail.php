...
use Exception;
use Net_URL2;
use OpenDxp\Mail as MailClient;
use OpenDxp\Model;
use OpenDxp\Tool;
...
class Mail
{
    ...
    public static function getDebugInformation(string $type, MailClient $mail): string
    {
        ...
        if ($type === 'html') {
            ...
            $debugInformation .= '<tr><td class="opendxp_label_column">From:</td><td>';
            ...
            foreach (['To', 'Cc', 'Bcc', 'ReplyTo'] as $key) {
                ...
                if ($addresses) {
                    $debugInformation .= '<tr><td class="opendxp_label_column">' . $key . ': </td>';
...
.opendxp_debug_information td, .opendxp_debug_information th{
    padding: 5px 10px;
    vertical-align:top;
    border: 1px solid #ccc;

}
.opendxp_debug_information th{
    text-align:center;
    font-weight:bold;
}
.opendxp_label_column{
    width:80px;
}
...
CSS;
    ...
    public static function setAbsolutePaths(string $string, ?Model\Document $document = null, ?string $hostUrl = null): string
    {
        ...
        if (!$hostUrl && $document) {
            ...
            $site = \OpenDxp\Tool\Frontend::getSiteForDocument($document);
            if ($site) {
                $hostUrl = \OpenDxp\Tool::getRequestScheme() . '://' . $site->getMainDomain();
        ...
        if ($matches[0]) {
            ...
            foreach (array_keys($matches[0]) as $key) {
                ...
                if (stream_is_local($path)) {
                    ...
                    $fileContent = \OpenDxp\Tool::getHttpData($path);
                    $fileInfo = [
    ...
    public static function getNormalizedFileInfo(string $path, ?Model\Document $document = null): array
    {
        ...
        $fileInfo['filePathNormalized'] = OPENDXP_WEB_ROOT . preg_replace('@^/cache-buster\-\d+\/@', '/', str_replace($hostUrl, '', $fileInfo['fileUrlNormalized']));
