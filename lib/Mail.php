...
class Mail extends Email
{
    ...
    /**
     * Initializes the mailer with the configured opendxp.email system settings
     *
     *
     * @internal
     */
    ...
    public function doRedirectMailsToDebugMailAddresses(): bool
    {
        ...
        return OpenDxp::inDebugMode() && $this->ignoreDebugMode === false;
    }
    ...
    private function setDocumentSettings(): static
    {
        ...
        if ($document instanceof Model\Document\Email) {
            ...
            $fromArray = \OpenDxp\Helper\Mail::parseEmailAddressField($document->getFrom());
            if ($fromArray) {
                [$from] = $fromArray;
    ...
    public function sendWithoutRendering(?MailerInterface $mailer = null): static
    {
        ...
        OpenDxp::getEventDispatcher()->dispatch($event, MailEvents::PRE_SEND);
        ...
        if ($this->loggingIsEnabled()) {
            if (OpenDxp::inDebugMode() && !$this->ignoreDebugMode) {
