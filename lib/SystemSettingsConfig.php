...
class SystemSettingsConfig
{
    ...
    {
        if (RuntimeCache::isRegistered('opendxp_system_settings_config')) {
            $config = RuntimeCache::get('opendxp_system_settings_config');
        } else {
            $config = self::get();
    ...
    {
        RuntimeCache::set('opendxp_system_settings_config', $config);
    }
    ...
    {
        $containerConfig = \OpenDxp\Config::getSystemConfiguration();
        $data = [
