...
use OpenDxp\Security\User\UserProvider;
...
class Authentication
{
    public static function authenticateSession(?Request $request = null): ?User
    {
        ...
        $token = $session->get('_security_opendxp_admin');
        $token = $token ? static::safelyUnserialize($token) : null;

        if ($token instanceof TokenInterface) {
            $token = static::refreshUser($token, OpenDxp::getContainer()->get(UserProvider::class));
            $user = $token->getUser();

            if ($user instanceof \OpenDxp\Security\User\User && self::isValidUser($user->getUser())) {
    ...
    {
        $secret = OpenDxp::getContainer()->getParameter('secret');
