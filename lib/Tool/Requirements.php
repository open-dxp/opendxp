...
final class Requirements
{
    ...
    public static function checkFilesystem(): array
    {
        ...
        // filesystem checks
        foreach ([OPENDXP_PRIVATE_VAR] as $varDir) {
            $varWritable = true;
            ...
            try {
                ...
                $checks[] = new Check([
                    'name' => str_replace(OPENDXP_PROJECT_ROOT, '', $varDir) . ' writeable',
    ...
    public static function checkMysql(Connection $db): array
    {
        ...
        try {
            $db->executeQuery('CREATE TABLE __opendxp_req_check (
        ...
        try {
            $db->executeQuery('UPDATE __opendxp_req_check SET field = :field, alter_field = :alter_field', [
                'field' => uniqid(),
                'alter_field' => uniqid(),
            ]);
        } catch (Exception $e) {
    ...
    public static function checkExternalApplications(): array
    {
        ...
        $checks[] = new Check([
            'name' => 'Composer',
            'state' => (bool) \OpenDxp\Tool\Console::getExecutable('composer') ? Check::STATE_OK : Check::STATE_ERROR,
        ]);
    ...
    public static function checkPhp(): array
    {
        ...
        if (class_exists('Imagick')) {
            $convertExecutablePath = \OpenDxp\Tool\Console::getExecutable('convert');
            ...
            $checks[] = new Check([
                ...
                'link' => 'https://github.com/open-dxp/opendxp/doc/19_Development_Tools_and_Details',
