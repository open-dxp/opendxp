...
class Text
{
            ...
            for ($i = 0; $i < $counter; $i++) {
                ...
                if ($element instanceof Element\ElementInterface) {
                    if ($matches[1][$i] == 'a') {
                        ...
                        if (!preg_match('/opendxp_disable_thumbnail="([^"]+)*"/', $oldTag)) {
                            if (!empty($config)) {
    ...
    private static function extractOpenDxpAttributes(string $html): array
    {
        //getting all links and images with opendxp_id and opendxp_type attribute with ignoring the order
        $pattern = '@<(a|img)[^>]*\bopendxp_id="([\d]+)"[^>]*\bopendxp_type="(asset|document|object)"|<(a|img)[^>]*\bopendxp_type="(asset|document|object)"[^>]*\bopendxp_id="([\d]+)"@msUi';
        ...
        for ($i = 0; $i < $counter; $i++) {
            ...
            $tag = $match[1] ?: $match[4];
            $opendxp_id = $match[2] ?: $match[6];
            $opendxp_type = $match[3] ?: $match[5];

            $results[0][$i] = $match[0];
            $results[1][$i] = $tag;
            $results[2][$i] = 'opendxp_id="' . $opendxp_id . '"';
            $results[3][$i] = 'opendxp_type="' .$opendxp_type . '"';
        }
