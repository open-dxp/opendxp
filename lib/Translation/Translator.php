    ...
    public function lazyInitialize(string $domain, string $locale): void
    {
        ...
        if (Translation::isAValidDomain($domain)) {
            if (!$catalogue = Cache::load($cacheKey)) {
                $data = ['__opendxp_dummy' => 'only_a_dummy'];
                $dataIntl = ['__opendxp_dummy' => 'only_a_dummy'];

                $list = new Translation\Listing();
                $list->setDomain($domain);

                $debugAdminTranslations = \OpenDxp\Config::getSystemConfiguration('general')['debug_admin_translations'] ?? false;
