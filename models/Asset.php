...
use OpenDxp\Model\Asset\Dao;
use OpenDxp\Model\Asset\Folder;
use OpenDxp\Model\Asset\Image\Thumbnail\Config as ThumbnailConfig;
use OpenDxp\Model\Asset\Listing;
use OpenDxp\Model\Asset\MetaData\ClassDefinition\Data\Data;
use OpenDxp\Model\Asset\MetaData\ClassDefinition\Data\DataDefinitionInterface;
use OpenDxp\Model\Element\DuplicateFullPathException;
use OpenDxp\Model\Element\ElementInterface;
use OpenDxp\Model\Element\Service;
use OpenDxp\Model\Element\Traits\ScheduledTasksTrait;
    ...
    public static function getById(int $id, array $params = []): ?static
    {
        ...
        if ($params['force'] || !($asset = Cache::load($cacheKey))) {
            ...
            try {
                ...
                $className = OpenDxp::getContainer()->get('opendxp.class.resolver.asset')->resolve($asset->getType());
                /** @var Asset $newAsset */
    ...
    public static function create(int $parentId, array $data = [], bool $save = true): Asset
    {
        ...
        if (
            ...
            if (array_key_exists('data', $data) || array_key_exists('stream', $data)) {
                $tmpFile = OPENDXP_SYSTEM_TEMP_DIRECTORY . '/asset-create-tmp-file-' . uniqid() . '.' . pathinfo($data['filename'], PATHINFO_EXTENSION);
                $mimeTypeGuessData = $tmpFile;

                if (!str_starts_with($tmpFile, OPENDXP_SYSTEM_TEMP_DIRECTORY)) {
        ...
        $className = OpenDxp::getContainer()->get('opendxp.class.resolver.asset')->resolve($type)
