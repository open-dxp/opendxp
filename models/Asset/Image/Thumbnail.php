...
use OpenDxp\Model\Asset\Thumbnail\ImageThumbnailTrait;
use OpenDxp\Model\Exception\NotFoundException;
use OpenDxp\Model\Exception\ThumbnailFormatNotSupportedException;
use OpenDxp\Model\Exception\ThumbnailMaxScalingFactorException;
use OpenDxp\Tool;
...
final class Thumbnail implements ThumbnailInterface
{
    ...
    protected function hasListeners(string $eventName): bool
    {
        if (!isset(self::$hasListenersCache[$eventName])) {
            self::$hasListenersCache[$eventName] = OpenDxp::getEventDispatcher()->hasListeners($eventName);
        }
    ...
    public function generate(bool $deferredAllowed = true): void
    {
        ...
        if ($this->hasListeners(AssetEvents::IMAGE_THUMBNAIL)) {
            ...
            OpenDxp::getEventDispatcher()->dispatch($event, AssetEvents::IMAGE_THUMBNAIL);
        }
    }
    ...
    public function getImageTag(array $options = [], array $removeAttributes = []): string
    {
        ...
        if (empty($altText) && (!isset($options['disableAutoAlt']) || !$options['disableAutoAlt'])) {
            ...
            $customAlt = OpenDxp\Config::getSystemConfiguration('assets')['metadata']['alt'];
        ...
        if (!isset($options['disableAutoCopyright']) || !$options['disableAutoCopyright']) {
            ...
            $customCopyright = OpenDxp\Config::getSystemConfiguration('assets')['metadata']['copyright'];
