...
use OpenDxp\Model\Asset\Image\ThumbnailInterface;
...
class Service extends Model\Element\Service
{
    ...
    public function copyRecursive(Asset $target, Asset $source, bool $initial = true): Asset|Folder|null
    {
        ...
        OpenDxp::getEventDispatcher()->dispatch($event, AssetEvents::PRE_COPY);
        ...
        OpenDxp::getEventDispatcher()->dispatch($event, AssetEvents::POST_COPY);
    ...
    public static function getImageThumbnailByArrayConfig(array $config): null|ThumbnailInterface|Asset\Video\ImageThumbnailInterface|Asset\Document\ImageThumbnailInterface|array
    {
        ...
        if (ltrim($asset->getPath(), '/') === ltrim($prefix, '/')) {
            ...
            $thumbnailConfigClass = 'OpenDxp\\Model\\Asset\\' . ucfirst($config['type']) . '\\Thumbnail\Config';
    ...
    {
        // See `_opendxp_service_thumbnail` in `CoreBundle\config\routing.yaml`
