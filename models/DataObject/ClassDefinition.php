...
use OpenDxp\Model\DataObject\ClassDefinition\Data\ManyToOneRelation;

/**
 * @method \OpenDxp\Model\DataObject\ClassDefinition\Dao getDao()
 */
final class ClassDefinition extends Model\AbstractModel implements ClassDefinitionInterface
{
    ...
    public function save(bool $saveDefinitionFile = true): void
    {
        ...
        foreach ($fieldDefinitions as $fd) {
            if ($fd instanceof \OpenDxp\Model\DataObject\ClassDefinition\Data\ClassSavedInterface) {
                $fd->classSaved($this);
            }
        }
    ...
    {
        OpenDxp::getContainer()->get(PHPClassDumperInterface::class)->dumpPHPClasses($this);
     ...
     * @internal
     *
     * with OPENDXP_CLASS_DEFINITION_WRITABLE set, it globally allow/disallow creation and change in classes
     * when the ENV is not set, it allows modification and creation of new in classes in /var/classes but disables modification of classes in config/opendxp/classes
