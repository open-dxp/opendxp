 ...
 *
 * @package OpenDxp\Model\DataObject\ClassDefinition\Data
 *
...
class EncryptedField extends Data implements ResourcePersistenceAwareInterface, TypeDeclarationSupportInterface, EqualComparisonInterface, VarExporterInterface, NormalizerInterface, LayoutDefinitionEnrichmentInterface
{
    ...
    private function encrypt(mixed $data, ?Model\DataObject\Concrete $object, array $params): ?string
    {
        if (!is_null($data)) {
            ...
            try {
                ...
                throw new Exception('Could not find config "opendxp.encryption.secret". Please run "vendor/bin/generate-defuse-key" from command line and add the result to config/config.yaml');
            }
    ...
    {
        return $this->delegate ? $this->delegate->getPhpdocInputType() . '|\\OpenDxp\\Model\\DataObject\\Data\\EncryptedField' : null;
    }

    public function getPhpdocReturnType(): ?string
    {
        return $this->delegate ? $this->delegate->getPhpdocReturnType() . '|\\OpenDxp\\Model\\DataObject\\Data\\EncryptedField' : null;
    }
