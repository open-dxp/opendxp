...
class Objectbricks extends Data implements CustomResourcePersistingInterface, TypeDeclarationSupportInterface, NormalizerInterface, DataContainerAwareInterface, IdRewriterInterface, PreSetDataInterface
{
    ...
    public function getDataFromEditmode(mixed $data, ?DataObject\Concrete $object = null, array $params = []): Objectbrick
    {
        ...
        if (empty($container)) {
            ...
            $containerClass = '\\OpenDxp\\Model\\DataObject\\' . ucfirst($className) . '\\' . ucfirst($this->getName());
        ...
        if (is_array($data)) {
            foreach ($data as $collectionRaw) {
                ...
                if (empty($brick)) {
                    $brickClass = '\\OpenDxp\\Model\\DataObject\\Objectbrick\\Data\\' . ucfirst($collectionRaw['type']);
    ...
    public function load(Localizedfield|AbstractData|\OpenDxp\Model\DataObject\Objectbrick\Data\AbstractData|Concrete $object, array $params = []): mixed
    {
        $classname = '\\OpenDxp\\Model\\DataObject\\' . ucfirst($object->getClass()->getName()) . '\\' . ucfirst($this->getName());
    ...
    public function delete(Localizedfield|AbstractData|\OpenDxp\Model\DataObject\Objectbrick\Data\AbstractData|Concrete $object, array $params = []): void
    {
    ...
    public function getGetterCode(DataObject\Objectbrick\Definition|DataObject\ClassDefinition|DataObject\Fieldcollection\Definition $class): string
    {
        ...
        $classname = '\\OpenDxp\\Model\\DataObject\\' . ucfirst($class->getName()) . '\\' . ucfirst($this->getName());
        ...
        $code .= "\t\t" . 'if (\OpenDxp\Tool::classExists("' . str_replace('\\', '\\\\', $classname) . '")) {' . "\n";
    ...
    public function getDiffDataFromEditmode(array $data, ?DataObject\Concrete $object = null, array $params = []): mixed
    {
        ...
        foreach ($data as $item) {
            ...
            if (!$brick) {
                ...
                $brickClass = '\\OpenDxp\\Model\\DataObject\\Objectbrick\\Data\\' . ucfirst($brickname);
                $brick = new $brickClass($object);
            }
