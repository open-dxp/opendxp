...
class Video extends Model\Document\Editable implements IdRewriterInterface
{
    ...
    private function getAssetCode(bool $inAdmin = false): string
    {
        ...
        if (!\OpenDxp\Video::isAvailable() || !$thumbnailConfig) {
    ...
    private function getYoutubeCode(bool $inAdmin = false): string
    {
        ...
        if ($inAdmin && isset($config['editmodeImagePreview']) && $config['editmodeImagePreview'] === true) {
            return '<div id="opendxp_video_' . $this->getName() . '" class="opendxp_editable_video '. ($config['class'] ?? '') .'">
    ...
    private function getHtml5Code(
        ...
        if ($video) {
            $code .= '<div id="opendxp_video_' . $this->getName() . '" class="opendxp_editable_video">' . "\n";
        ...
        <div id="opendxp_video_' . $this->getName() . '" class="opendxp_editable_video">
            <style type="text/css">
                #' . $uid . ' .opendxp_editable_video_progress_status {
                    ...
                    background:#fff url(/bundles/opendxpadmin/img/video-loading.gif) center center no-repeat;
                    width:66px;
                    height:66px;
                    padding:20px;
            ...
            <div class="opendxp_editable_video_progress" id="' . $uid . '">
    ...
    private function getEmptyCode(): string
    {
        ...
        return '<div id="opendxp_video_' . $this->getName() . '" class="opendxp_editable_video"><div class="opendxp_editable_video_empty" id="' . $uid . '" style="width: ' . $this->getWidthWithUnit() . '; height: ' . $this->getHeightWithUnit() . ';"></div></div>';
    }
    ...
    /**
     * TODO OpenDXP 2: Change empty string return to null
     */
