...
/**
 * @method \OpenDxp\Model\Translation\Dao getDao()
 */
final class Translation extends AbstractModel
{
    ...
    protected ?HtmlSanitizerInterface $openDxpTranslationSanitizer = null;

    public function getTranslationSanitizer(): HtmlSanitizerInterface
    {
        return $this->openDxpTranslationSanitizer ??= OpenDxp::getContainer()->get(Tool\Text::OPENDXP_TRANSLATION_SANITIZER_ID);
    }
    ...
    {
        $translationsConfig = \OpenDxp\Config::getSystemConfiguration('translations');
     ...
     * The CSV file has to have the same format as an OpenDxp translation-export-file
     *
    ...
    public static function importTranslationsFromFile(string $file, string $domain = self::DOMAIN_DEFAULT, bool $replaceExistingTranslations = true, ?array $languages = null, ?stdClass $dialect = null): array
    {
        ...
        if (is_readable($file)) {
            ...
            $importFile = OPENDXP_SYSTEM_TEMP_DIRECTORY . '/import_translations';
            $filesystem->dumpFile($importFile, $tmpData);

            $importFileOriginal = OPENDXP_SYSTEM_TEMP_DIRECTORY . '/import_translations_original';
            $filesystem->dumpFile($importFileOriginal, $tmpData);

            // determine csv type if not set
            if (empty($dialect)) {
                $dialect = Tool\Admin::determineCsvDialect(OPENDXP_SYSTEM_TEMP_DIRECTORY . '/import_translations_original');
            }

            //read data
            $data = [];
            if (($handle = fopen(OPENDXP_SYSTEM_TEMP_DIRECTORY . '/import_translations', 'r')) !== false) {
